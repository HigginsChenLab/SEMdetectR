utils::globalVariables(c("houseman_dmrs", "houseman_dmrs_locs"))
#' Estimate Cell Proportions from Methylation Data
#'
#' This function estimates the proportions of different cell types in a mixed tissue sample using DNA methylation data. It is based on an algorithm originally proposed by Houseman et al. (2012), adapted for use in SEMdetectR.
#'
#' @param B A matrix of beta values for methylation data.
#' @param top_n The number of top differentially methylated regions (DMRs) to consider.
#' @param mc.cores The number of cores to use for parallel processing (not used actually).
#' @param cell.coefs Coefficients for cell types, if pre-calculated.
#'
#' @return A matrix of estimated cell type proportions for each sample.
#'
#' @keywords internal
estimate.cell.proportions <-  function (B, top_n = 500, mc.cores = 2, cell.coefs = NULL)
{
  if (is.null(cell.coefs)) {
    if (nrow(B) <= 2e+05) {
      if (length(grep("^cg", rownames(B), value = TRUE,
                      perl = TRUE)) >= length(grep("^chr", rownames(B),
                                                   value = TRUE, perl = TRUE))) {
        dmr.coefs = houseman_dmrs
      }
      else {
        dmr.coefs = houseman_dmrs
      }
  ...
  if(dim(dmr.coefs)[1] == 0) {
    stop("0 CpGs can be used to infer cell counts, which is required for the RF models. Aborting...")
  }
  ...
  omega.mix = projectWBC(B[dmrs, ], dmr.coefs[dmrs, ], contrastWBC = Lwbc,
